...
public:
  ...
  { 
	...
	clEnqueueMigrateMemObjects(mQueue, 1, mSrcBuf, 0, 0, nullptr,  &req->mEvent[0]);	
  	...
  	clSetKernelArg(mKernel, 1, sizeof(float),        &factor);
  	clSetKernelArg(mKernel, 2, sizeof(cl_mem),       &mSrcBuf[1]);
  	clSetKernelArg(mKernel, 3, sizeof(unsigned int), &width);
  	clSetKernelArg(mKernel, 4, sizeof(unsigned int), &height);
  	clSetKernelArg(mKernel, 5, sizeof(unsigned int), &stride);
  	clSetKernelArg(mKernel, 6, sizeof(cl_mem),       &mDstBuf[0]);
...
int main(int argc, char** argv)
{
	...
	parser.addSwitch("--filter", "-f", "Filter type (0-5)", "0");
	...
	if ((coeffs<0) || (coeffs>5)) {
		std::cout << std::endl;	
		std::cout << "ERROR: Supported filter type values are [0:5]" << std::endl;
		exit(1);
	}
	...
	// Retrieve filter factor
	float factor = filterFactors[coeffs];
  ...
  auto fpga_begin = std::chrono::high_resolution_clock::now();
	...
	for(int xx=0; xx<numRuns; xx++) 
	{
		...
		request[xx*3+0] = Filter(coeff.data(), factor, y_src.data(), width, height, stride, y_dst.data());
		request[xx*3+1] = Filter(coeff.data(), factor, u_src.data(), width, height, stride, u_dst.data());
		request[xx*3+2] = Filter(coeff.data(), factor, v_src.data(), width, height, stride, v_dst.data());
  ...
  auto cpu_begin = std::chrono::high_resolution_clock::now();
	...
	for(int xx=0; xx<numRuns; xx++) 
	{
		...
		Filter2D(filterCoeffs[coeffs], factor, y_src.data(), width, height, stride, y_ref.data());
		Filter2D(filterCoeffs[coeffs], factor, u_src.data(), width, height, stride, u_ref.data());
		Filter2D(filterCoeffs[coeffs], factor, v_src.data(), width, height, stride, v_ref.data());
	}
